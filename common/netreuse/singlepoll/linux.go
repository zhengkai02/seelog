...
func criticalError(msg string, err error) {
	...
	log.Fatalf("This is critical error, please report it at https://github.com/jbenet/go-reuseport/issues/new")
	log.Fatalf("Bailing out. You are on your own. Good luck.")
...
func poller(epfd int, evfd *eventfd.EventFD) {
	for {
		...
		default:
			// log
			log.Fatalf("EpollWait returned error: %s. Continuing.", err.Error())
			continue
		}
